...
use overlay::set_global_registry;
...
use oca_ast::ast::{
    CaptureContent, Command, CommandType, Content, NestedValue, OCAAst, ObjectKind,
};
...
pub struct OCABox {
    ...
    /// TODO Rename it to properties
    pub meta: Option<HashMap<Option<Language>, HashMap<String, String>>>,
...
impl OCABox {
    ...
    pub fn add_overlay(&mut self, overlay_name: String, content: Option<Content>) {
        let mut overlay = Overlay::new(overlay_name);
        overlay.properties = content.unwrap().properties;
    ...
    fn generate_overlays(&mut self) -> Vec<Overlay> {
...
impl OCABundle {
    pub fn new(capture_base: CaptureBase, overlays: Vec<Overlay>, registry: OverlayLocalRegistry) -> Self {
...
mod tests {
    ...
    fn build_oca_bundle() {
        ...
        todo!();
        // oca.add_overlay("Meta", properties, attributes);
